...
export default async function FacturasPage({ 
  ...
  const total = factura.bills.reduce((acc, bill) => acc + bill.monto_total, 0) + (factura.extras?.reduce((acc, extra) => acc + extra.monto, 0) ?? 0);
  ...
  return (
    <div className="container mx-auto py-6 px-4">
      ...
      <div className="flex flex-col md:flex-row md:justify-between gap-1 mb-6">
        ...
        <h2 className="text-lg md:text-2xl font-bold ">{formatearFechaParaMostrar(factura.fecha)}</h2>
        <h2 className="text-lg md:text-2xl font-bold ">Total: ${formatearMonto(total)}</h2>
      </div>
