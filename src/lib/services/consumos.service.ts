...
import { ConsumoExtra, ConsumoExtraAplicado, NuevoConsumoExtraAAplicar } from "../types/consumos";
...
export async function crearConsumosExtra(extras: Omit<ConsumoExtra, "id">[]): Promise<ConsumoExtra[]> {
...
export async function insertarConsumoExtra(extra: Omit<ConsumoExtraAplicado, "id" | "factura_id">, factura_id: string): Promise<ConsumoExtraAplicado> {
    const { data, error } = await supabase()
    .from("bill_extras")
    .insert({
        ...extra,
...
export async function actualizarConsumoExtra(id: number, extra: Omit<ConsumoExtra, "id">) {
    const { data, error } = await supabase()
    .from("extras")
    .update(extra)
    .eq("id", id)
...
export async function aplicarConsumoExtraAFacturas(extras: NuevoConsumoExtraAAplicar[], fecha: Date) {
  console.log("🚀 ~ aplicarConsumoExtraAFacturas ~ fecha:", fecha)
  console.log("🚀 ~ aplicarConsumoExtraAFacturas ~ extras:", extras)
  const extrasAplicados: ConsumoExtraAplicado[] = [];
  const errores: string[] = [];

  for (const extra of extras) {
    console.log("🚀 ~ aplicarConsumoExtraAFacturas ~ extra:", extra)
    try {
      ...
      if (!facturaMensual || facturaMensual.length === 0) {
        ...
        console.log("🚀 ~ aplicarConsumoExtraAFacturas ~ contract:", contract)

        if (contractError) {
          errores.push(`No se encontró contrato para el legajo ${extra.legajo}`);
          continue;
        }
        console.log("🚀 ~ aplicarConsumoExtraAFacturas ~ extra.legajo:", extra.legajo)
      ...
      const { data: descuentoExistente, error: descuentoError } = await supabase()
        .from("extras")
        .select("*")
